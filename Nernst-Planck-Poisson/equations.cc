...
void variableAttributeLoader::loadVariableAttributes(){
    ...
    set_dependencies_gradient_term_RHS(0, "grad(c), grad(phi), c");
    ...
    set_dependencies_gradient_term_LHS(1, "grad(change(phi))");
...
scalargradType eqx_c = (userInputs.dtValue*(constV(-D)*cx))+((userInputs.dtValue*constV(-D*z*F)*c*phix)/(constV(R*T)));
...
void customPDE<dim,degree>::nonExplicitEquationRHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
 ...
 scalarvalueType eq_phi = ((constV(-F*z)*c))/constV(eps);

 scalargradType eq_phix = phix;

 // --- Submitting the terms for the governing equations ---

 variable_list.set_scalar_value_term_RHS(1,eq_phi);
 variable_list.set_scalar_gradient_term_RHS(1,eq_phix);
...
void customPDE<dim,degree>::equationLHS(variableContainer<dim,degree,dealii::VectorizedArray<double> > & variable_list,
		dealii::Point<dim, dealii::VectorizedArray<double> > q_point_loc) const {
			...
			scalargradType DPhix = variable_list.get_change_in_scalar_gradient(1);
			scalargradType rDPhix = -DPhix;

			variable_list.set_scalar_gradient_term_LHS(1,rDPhix);
